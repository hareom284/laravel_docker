...
<body>
    ...
    @php
        $originalIndex = [];
        $termsContent = strip_tags($quotationList->terms);
        ...
        <div class="pdf-content">
            <div class="avoid-break">
                ...
                @foreach ($sortQuotation as $index => $item)
                    <div key="{{ $item['section_id'] }}" class="avoid-break {{ $item['is_page_break'] ? 'page' : '' }}">
                        <table border="1"
                                    ...
                                    <tr key="{{ $emptyAOW['id'] }}">
                                        ...
                                        @if (!empty($emptyAOW['items']))
                                            @foreach ($emptyAOW['items'] as $subItem)
                                                @include('pdf.CANCELLATION.Metis.cancellation_subitems', [
                                                    'item' => $subItem,
                                                    'level' => 1,
                                                ])
                                            @endforeach
                                        ...
                                        <tr key="{{ $hasAOW['id'] }}">
                                            ...
                                            @if (!empty($hasAOW['items']))
                                                @foreach ($hasAOW['items'] as $subItem)
                                                    @include('pdf.QUOTATION.Metis.quotation_subitems', [
                                                        'item' => $subItem,
                                                        'level' => 1,
                                                    ])
                                                @endforeach

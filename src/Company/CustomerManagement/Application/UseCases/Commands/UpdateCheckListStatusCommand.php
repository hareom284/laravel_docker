...
class UpdateCheckListStatusCommand implements CommandInterface
{
    ...
    {
        $checkList = $this->repository->updateCheckListStatus($this->data);

        if ($this->data && $this->data['customer_id']) {
            $customer = CustomerEloquentModel::find($this->data['customer_id']);
            if ($customer) {
                $newCustomer = $customer->transition('completeChecklist');

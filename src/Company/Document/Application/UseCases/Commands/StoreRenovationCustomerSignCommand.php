...
class StoreRenovationCustomerSignCommand implements CommandInterface
{
    ...
    public function execute(): mixed
    {
        ...
        if ($document->type == 'QUOTATION') {
            ...
            $enable_payment_term = GeneralSettingEloquentModel::where('setting', "enable_payment_terms")
            ->where('value', "true")
            ->first();
            $enableDataMigrationMode = GeneralSettingEloquentModel::where('setting', "enable_data_migration_mode")
            ->where('value', "true")
            ->first();
            if($firstCustomerId && $payment_terms && $enable_payment_term && !$enableDataMigrationMode){

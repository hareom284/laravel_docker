...
class QuotationTemplateItemsRepository implements QuotationTemplateItemsRepositoryInterface
{
    ...
    public function duplicateTemplate($request)
    {
       //id for duplicate
        ...
        $newTemplate =  QuotationTemplatesEloquentModel::create([
            ...
            'quotation_template_category_id' => $quotationTemplate->quotation_template_category_id
    ...
    public function retrieveAllTemplate($salespersonId, $contract)
    {
        if (isset($contract) && $contract == 'true') {
            $signedContracts = RenovationDocumentsEloquentModel::where('type', 'QUOTATION')
            ->whereHas('projects',function($query) use($salespersonId){
                $query->whereHas('salespersons', function($query) use($salespersonId){
                    $query->where('salesperson_id', $salespersonId);
                });
            })
            ->whereNotNull('signed_date')
            ->get();
            return $signedContracts->map(function ($contract) {
                $blockNum = $contract->projects->property->block_num ?? '';
                $streetName = $contract->projects->property->street_name ?? '';
                $unitNum = $contract->projects->property->unit_num ?? '';
                $postalCode = $contract->projects->property->postal_code ?? '';
                $postalSuffix = $postalCode ? " S({$postalCode})" : '';
                $address = trim("{$blockNum} {$streetName} {$unitNum}{$postalSuffix}");
                return [
                    "id" => $contract->id,
                    "name" => $address ?: 'N/A', // Fallback when the address is empty
                    "quotation_template_category_id" => null,
                    "contract" => true,
                ];
            });
        } else {
    ...
    public function retrieveTemplate($templateId)
    {
        ...
        $data->quotation_template_category_id = isset($quotationTemplate) ? $quotationTemplate->quotation_template_category_id : null;
    ...
    public function createTemplate($requestQuotationTemplate)
    {
        ...
        $quotationTemplate =  QuotationTemplatesEloquentModel::create([
            ...
            'quotation_template_category_id' => $requestQuotationTemplate['quotation_template_category_id'] ?? null
        ]);
    ...
    public function updateTemplate($quotationTemplate)
    {
        ...
        $requestCategoryId = $quotationTemplate['quotation_template_category_id'] ?? null;
        ...
        QuotationTemplatesEloquentModel::where('id', $templateId)->update([
            'name' => $templateName,
            'quotation_template_category_id' => $requestCategoryId
        ]);
        ...
        foreach ($parentObj['items'] as $requestItem) {
            if (isset($requestItem['id'])) {
                ...
                if ($item) { // Update existing item
                    ...
                    $item->sub_description = $requestItem['sub_description'] ?? '';
                ...
                $item = QuotationTemplateItemsEloquentModel::Create([
                    ...
                    'salesperson_id' => $salespersonId,
                    'parent_id' => isset($parentObj['id']) && isset($requestItem['parent_item_index']) ? $parentObj['id'] : null, // parent_item_index is an attribute passed from frontend, if this attribute exists, it means this particular item belongs to another item
                    'sub_description' => $requestItem['sub_description'] ?? '',

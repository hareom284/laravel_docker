...
class ProjectRequirementRepository implements ProjectRequirementRepositoryInterface
{
    ...
    public function getProjectRequirements($project_id)
    {
        ...
        $requirements = ProjectRequirementResource::collection($prEloquent);
    ...
    public function findRequirementById(int $id): ProjectRequirementData
    {
        ...
        return ProjectRequirementData::fromEloquent($prEloquent, true, true, true);
    }
    ...
    public function update(array $data)
    {
        ...
        $requirementEloquent = ProjectRequirementEloquentModel::query()->where('id',$data['id'])->first();

        if(isset($data['document_file'])) {

            dd("Hello");

            $fileName =  time() . '.' . $data['document_file']->getClientOriginalExtension();

            $filePath = 'project_requirement/' . $fileName;

            Storage::disk('public')->put($filePath, file_get_contents($data['document_file']));

            $documentFile = $fileName;
        } else {
            // Set null if don't exists
            $documentFile = $data['original_document'] ? $data['original_document'] : null;
        }

        $requirementEloquent->document_file = $documentFile;
        $requirementEloquent->title = $data['name'];
        $requirementEloquent->project_id = $data['project_id'];

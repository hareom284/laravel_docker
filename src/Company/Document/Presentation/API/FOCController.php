...
use Src\Company\Document\Application\UseCases\Commands\StoreFOCDocumentCommand;
use Src\Company\Document\Application\UseCases\Queries\FindFOCItemsQuery;
use Src\Company\Document\Application\UseCases\Queries\GetCountListsFOCQuery;
...
class FOCController extends Controller
{
    ...
    public function countLists($project_id)
    {
        try {
            ...
            $result = (new GetCountListsFOCQuery($project_id))->handle();
    ...
    public function oldgetAllFOCItems($project_id)
    {
        try {

            $final_result = (new FindFOCItemsQuery($project_id))->handle();
           
            return response()->json($final_result, Response::HTTP_OK);
        } catch (\DomainException $domainException) {
            return response()->json(['error' => $domainException->getMessage()], Response::HTTP_UNPROCESSABLE_ENTITY);
        } catch (UnauthorizedUserException $e) {
            return response()->json(['error' => $e->getMessage()], Response::HTTP_UNAUTHORIZED);
    ...
    public function getAllFOCitems($project_id)
    {
        try {

            $foc_items = (new FindFOCItemsQuery($project_id))->handle();

            return response()->json($foc_items, Response::HTTP_OK);
        } catch (\DomainException $domainException) {
            return response()->json(['error' => $domainException->getMessage()], Response::HTTP_UNPROCESSABLE_ENTITY);
        } catch (UnauthorizedUserException $e) {
            return response()->json(['error' => $e->getMessage()], Response::HTTP_UNAUTHORIZED);
    ...
    public function store(StoreProjectQuotationRequest $request): JsonResponse
    {
        ...
        try {
            ...
            $renovation_documents_data = (new StoreFOCDocumentCommand($renovation_documents, $data))->execute();
